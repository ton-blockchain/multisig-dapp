const tonweb = {
    'main': new TonWeb(new TonWeb.HttpProvider('https://toncenter.com/api/v2/jsonRPC', {apiKey: '1b312c91c3b691255130350a49ac5a0742454725f910756aff94dfe44858388e'})),
    'test': new TonWeb(new TonWeb.HttpProvider('https://testnet.toncenter.com/api/v2/jsonRPC', {apiKey: '41ac6f57294e0805f1836ac7feb2befd8cbad2d85c87093faf04e4d108499cf8'}))
}

const multisig_code = ''

const BITSHIFT32 = (new tonweb[localStorage.getItem('network')].utils.BN(2)).pow(new tonweb[localStorage.getItem('network')].utils.BN(32))

class MultisigContract extends tonweb[localStorage.getItem('network')].Contract {
    constructor (provider, options) {
        options.code = tonweb[localStorage.getItem('network')].boc.Cell.oneFromBoc(multisig_code)
        super(provider, options)
        this.deploy = () => tonweb[localStorage.getItem('network')].Contract.createMethod(provider, this.createInitExternalMessage())
    }

    createDataCell () {
        var cell = new tonweb[localStorage.getItem('network')].boc.Cell()
        cell.bits.writeUint(this.options.wallet_id, 32)
        cell.bits.writeUint(this.options.n, 8)
        cell.bits.writeUint(this.options.k, 8)
        cell.bits.writeUint(0, 64)
        cell.bits.writeBit(1)
        cell.refs.push(this.options.owner_infos)
        cell.bits.writeBit(0)
        return cell
    }

    async createInitExternalMessage () {
        const {stateInit, address, code, data} = await this.createStateInit()

        const body = new tonweb[localStorage.getItem('network')].boc.Cell()
        const signingMessage = new tonweb[localStorage.getItem('network')].boc.Cell()

        const header = tonweb[localStorage.getItem('network')].Contract.createExternalMessageHeader(address)
        const externalMessage = tonweb[localStorage.getItem('network')].Contract.createCommonMsgInfo(header, stateInit, body)

        return {
            address: address,
            message: externalMessage,

            body,
            signingMessage,
            stateInit,
            code,
            data,
        }
    }
}

const newMultisig = async (pubkeys, wc, wallet_id, k) => {
    if (pubkeys.length < 2) {
        alert('Add at least 2 owners!')
        return
    }

    x = new tonweb[localStorage.getItem('network')].boc.HashMap(8)

    for (let i = 0; i < pubkeys.length; i++) {
        x.elements[i] = [tonweb[localStorage.getItem('network')].utils.hexToBytes(pubkeys[i]), i]
    }

    owner_infos = await x.serialize(
        k => {
            let key = new tonweb[localStorage.getItem('network')].boc.Cell()
            key.bits.writeUint(k, 8)
            return key
        },
        v => {
            let val = new tonweb[localStorage.getItem('network')].boc.Cell()
            val.bits.writeBytes(v[0])
            val.bits.writeUint(v[1], 8)
            return val
        }
    )

    const contract = new MultisigContract(tonweb[localStorage.getItem('network')].provider, {
        wc: wc,
        wallet_id: wallet_id,
        k: k,
        n: pubkeys.length,
        owner_infos: owner_infos
    })

    return contract
}

const getQueryId = () => {
    const time = new tonweb[localStorage.getItem('network')].utils.BN(Math.floor(Date.now() / 1000 + 7200))
    return time.mul(BITSHIFT32)
}

const rootSignOrder = async (owner_id, order) => {
    var cell = new tonweb[localStorage.getItem('network')].boc.Cell()
    cell.bits.writeUint(owner_id, 8)
    cell.bits.writeBit(0)
    cell.bits.writeUint(window.multisig_wallet_id, 32)
    cell.writeCell(order)
    const hash = await cell.hash()

    const signature = await ton.send(
        'ton_rawSign', [{ data: tonweb[localStorage.getItem('network')].utils.bytesToHex(hash) }]
    )

    var signedCell = new tonweb[localStorage.getItem('network')].boc.Cell()
    signedCell.bits.writeBytes(tonweb[localStorage.getItem('network')].utils.hexToBytes(signature))
    signedCell.writeCell(cell)

    return signedCell
}

const createExternalMessage = async (boc) => {
    var msg = new tonweb[localStorage.getItem('network')].boc.Cell()
    msg.bits.writeUint(2, 2)
    msg.bits.writeAddress(undefined)
    msg.bits.writeAddress(new tonweb[localStorage.getItem('network')].Address(window.multisig_address))
    msg.bits.writeCoins(0)
    msg.bits.writeBit(0)
    msg.bits.writeBit(1)
    msg.refs.push(boc)
    return msg
}

const addSignature = async (owner_id, cell) => {
    console.log('addSignature call')

    var signatures = new tonweb[localStorage.getItem('network')].boc.HashMap(8)
    await signatures.loadHashMapX2Y(cell, s => tonweb[localStorage.getItem('network')].boc.CellParser.loadUint(s, 8), s => tonweb[localStorage.getItem('network')].boc.CellParser.loadBits(s, 512))
    console.log('hash')
    const hash = await cell.bits.readBits(256)
    console.log('done')
    const signature = await ton.send(
        'ton_rawSign', [{ data: tonweb[localStorage.getItem('network')].utils.bytesToHex(hash) }]
    )
    signatures.elements[owner_id] = signature

    console.log(signatures)

    const signaturesCell = await signatures.serialize(
        k => {
            let key = new tonweb[localStorage.getItem('network')].boc.Cell()
            key.bits.writeUint(k, 8)
            return key
        },
        v => {
            let val = new tonweb[localStorage.getItem('network')].boc.Cell()
            val.bits.writeBytes(v)
            return val
        }
    )

    console.log(signaturesCell)

    var signedCell = new tonweb[localStorage.getItem('network')].boc.Cell()
    signedCell.writeCell(signaturesCell)
    signedCell.bits.writeBytes(hash)
    signedCell.writeCell(cell)

    console.log('addSignature ret')

    return signedCell
}

const signAndSend = async (boc) => {
    console.log(boc)
    var signedOrder = await rootSignOrder(window.multisig_owner_id, boc)
    var message = await createExternalMessage(signedOrder)
    return await tonweb[localStorage.getItem('network')].sendBoc(await message.toBoc(false))
}

const createInternalMessage = async (destAddr, amount, bounce, comment) => {
    var msg = new tonweb[localStorage.getItem('network')].boc.Cell()
    msg.bits.writeBit(0) // int_msg_info
    msg.bits.writeBit(1) // ihr_disabled
    msg.bits.writeBit(bounce) // bounce
    msg.bits.writeBit(0) // bounced
    msg.bits.writeAddress(undefined) // src
    msg.bits.writeAddress(new tonweb[localStorage.getItem('network')].Address(destAddr)) // dest
    msg.bits.writeCoins(amount) // value
    msg.bits.writeUint(0, 1 + 4 + 4 + 64 + 32 + 1)
    msg.bits.writeBit(1)
    var body = new tonweb[localStorage.getItem('network')].boc.Cell()
    body.bits.writeUint(0, 32)
    body.bits.writeString(comment)
    msg.refs.push(body)
    return msg
}

const createOrder = async (messages) => {
    var order = new tonweb[localStorage.getItem('network')].boc.Cell()

    order.bits.writeUint(getQueryId(), 64)

    for (const msg of messages) {
        order.bits.writeUint8(3) // message mode
        order.refs.push(msg)
    }

    return order
}